...
logs and does not know who visited.</p>
<p>It does read one thing: Cloudflare's aggregate counts for its own zones
&mdash; page views, unique visitors, and threats blocked, per site, per day.
That is where the numbers on the <a href="/status/">status page</a> come from.
It is a daily total and nothing else. It is not tied to a person, it cannot be,
and no script on this page produces it &mdash; the count is made by Cloudflare
while delivering the request, and read afterwards through their API.</p>
<p>Their handling is
governed by
