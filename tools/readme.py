...
# ── a README for a directory nobody should edit ──────────────
# The sentinel is load-bearing. plumhud, streamsniper, moonbeam and
# remapwrap have real hand-written READMEs of up to 239 lines sitting in
# the same shape of directory, and a generator that wrote README.md
# unconditionally would erase all four on its next run. Only a file that
# says it was generated is ever overwritten.
GENERATED_MARK = "<!-- generated by tools/buildsite.py -->"


def is_generated(path):
    """True only for a README this tool wrote. Absent counts as writable."""
    import io
    import os
    if not os.path.exists(path):
        return True
    with io.open(path, encoding="utf-8", errors="replace") as f:
        return GENERATED_MARK in f.read()


def product_readme(slug, title, tagline, facts):
    """What a developer landing in a generated directory needs to know.

    Not a second copy of the page - index.md is already that, and two
    descriptions of one product drift apart. The thing worth saying here
    is the thing the directory cannot say for itself: every file in it
    is written by a build, so an edit made here is not rejected, it is
    silently overwritten on the next run. That is the worse of the two
    failures, and it is invisible until the change you made is gone.
    """
    live = "https://f-keys.com/%s/" % slug
    lines = [
        "This directory is the %s page on f-keys.com. Nothing in it is" % title,
        "written by hand.",
        "",
        "```",
        ...
        "```",
        "",
        'All three come from `PAGES["%s"]` in `tools/buildsite.py`.' % slug,
    ...
    if facts:
        lines += ["| | |", "|---|---|"]
        lines += ["| %s | %s |" % (k, v) for k, v in facts]
        lines += [""]
    lines += ["[The page itself](%s)" % live, ""]
    return skeleton(title.upper(), tagline, "\n".join(lines)) \
        + "\n" + GENERATED_MARK + "\n"
